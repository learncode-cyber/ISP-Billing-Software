...
/**
 * SslcommerzGateway — concrete adapter, identical shape to BkashGateway.
 ...
 */
class SslcommerzGateway implements PaymentGatewayContract
{
    ...
    public function initiate(string $tenantId, string $invoiceId, float $amount): array
    {
        ...
        DB::table('billing.payment_gateway_transactions')->insert([
            ...
            'gateway_id' => DB::table('billing.payment_gateways')
                ->where('tenant_id', $tenantId)->where('provider', strtolower('Sslcommerz'))->value('id'),
            'invoice_id' => $invoiceId,
            'amount' => $amount,
            'status' => 'initiated',
            'initiated_at' => now(),
        ]);
        return ['transaction_id' => $txnId, 'redirect_url' => null, 'message' => 'Sslcommerz API integration pending.'];
    }
