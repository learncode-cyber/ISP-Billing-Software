...
/**
 * StripeGateway — concrete adapter, identical shape to BkashGateway.
    ...
    public function initiate(string $tenantId, string $invoiceId, float $amount): array
    {
        ...
        DB::table('billing.payment_gateway_transactions')->insert([
            ...
            'gateway_id' => DB::table('billing.payment_gateways')
                ->where('tenant_id', $tenantId)->where('provider', strtolower('Stripe'))->value('id'),
            'invoice_id' => $invoiceId,
            'amount' => $amount,
            'status' => 'initiated',
            'initiated_at' => now(),
        ]);
        return ['transaction_id' => $txnId, 'redirect_url' => null, 'message' => 'Stripe API integration pending.'];
    }
